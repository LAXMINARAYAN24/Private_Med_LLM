...
def synonyms_make(result_a, syn):
    result_a['cond_syn'] = ''
    result_a['synonyms'] = ''
    result_a['syn_dict'] = [{} for _ in range(len(result_a))]

    for description, synonyms in zip(syn['DESCRIPTION'], syn['synonym']):
        # Find matches where the 'condition' contains the 'DESCRIPTION'
        matches = (result_a['condition']).str.lower().str.contains(description, na=False)
        # Update the 'synonyms' column for rows where a match is found
        #print(description, synonyms)
        for idx in result_a[matches].index:
            # Append the synonyms to the 'synonyms' entry, handle NaN and conversion to string
            existing_synonyms = result_a.at[idx, 'synonyms']
            # Update the dictionary with the new description-synonyms pair
            syn_dict = result_a.at[idx, 'syn_dict']
            if isinstance(synonyms, str):
                syn_dict[description] = ast.literal_eval(synonyms)
            else:
                syn_dict[description] = synonyms

            # Save the updated dictionary back to the DataFrame
            result_a.at[idx, 'syn_dict'] = syn_dict
            #print(existing_synonyms)
            if existing_synonyms == '':
                if isinstance(synonyms, str):             
                    result_a.at[idx, 'synonyms'] = ast.literal_eval(synonyms)
                else:
                    result_a.at[idx, 'synonyms'] =synonyms
                    
#                result_a.at[idx, 'synonyms'] = ast.literal_eval(synonyms)
            else:
                # Assuming existing_synonyms is a list, otherwise convert it from string
                # If existing_synonyms is a string, convert it to list
                if isinstance(existing_synonyms, str):
                    existing_synonyms = ast.literal_eval(existing_synonyms)
                if isinstance(synonyms, str):
                    synonyms = ast.literal_eval(synonyms)

                result_a.at[idx, 'synonyms'] = existing_synonyms + synonyms


    #result_a['cond_syn'] = result_a.apply(lambda row: row['condition'] + ', ' + ', '.join(row['synonyms']) if bool(row['synonyms']) else row['condition'] + ', '.join(row['synonyms']), axis=1)
    result_a['cond_syn'] = result_a.apply(lambda row: row['condition'] if row['synonyms'] == '' else (row['condition'] + ', ' + ', '.join(row['synonyms'])), axis=1 )
    result_a['cond_syn'] =result_a['cond_syn'].apply(lambda x: ', '.join(list(set(x.split(', ')))))
...
def Eval3(output): # Exact Match + Synonym
    print("Start evaluation on Eval3 (Exact Match + Synonym) ")
    syn = pd.read_excel('../data/synonym_MedCat.xlsx')
    output = synonyms_make(output, syn)[['condition', 'cond_syn', 'opt_output', 'llama2_output', 'biomistral_output', 'prompt_class']]
    ...
    for i in tqdm(range(len(output))):
        target_list = output.condition[i].split(', ')
        cond_syn_list = output.cond_syn[i].split(', ')
        
        opt_em_n = 0
        llama2_em_n = 0
        biomistral_em_n = 0
        
        for j in range(len(cond_syn_list)):
            target = cond_syn_list[j].lower()
...
def Eval4(output):
    print("Start evaluation on Eval4 (Match + Split version + Synonym) ")
    syn = pd.read_excel('../data/synonym_MedCat.xlsx')
    output = synonyms_make(output, syn)[['condition', 'cond_syn', 'opt_output', 'llama2_output', 'biomistral_output', 'prompt_class']]
    ...
    for i in tqdm(range(len(output))):
        ...
        cond_syn_list = list(set(output.cond_syn[i].lower().replace(',', '').split(' ')))
        cond_syn_list = [x for x in cond_syn_list if len(x) >= 3]
